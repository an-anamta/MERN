  ...
  const data = [
    {
      ...
      content: 'Plot no-44, Behind City College, Vipul-Khand,Sector-5,Gomtinagar, lucknow, uttar pradesh'
    },
    {
      heading: 'Call Us',
      content: '+91 7283721001'
    },
    {
      heading: 'Mail Us',
      content: 'homehub@gmail.com'
    }
  ];
          ...
          <div className="col-md-6">
            <form onSubmit={handleSubmit((d) => handleContact(d))}>
              <div className="mb-3">
                ...
                <input type="text" className='form-control btncolor'   {...register('name')} placeholder="Enter Your Name" />
                {errors.name && <p className="text-danger">{errors.name.message}</p>}
              </div>

              <div className="mb-3">
                <label className="form-label"><FaEnvelope className="me-2" />Email</label>
                <input type="email" className='form-control btncolor'  {...register('email')} placeholder="Enter Your Email" />
                {errors.email && <p className="text-danger">{errors.email.message}</p>}
              </div>

              <div className="mb-3">
                <label className="form-label"><FaPhone className="me-2" />Phone</label>
                <input type="text" className='form-control btncolor' {...register('phone')} placeholder="Enter Your Mobile Number" />
                {errors.phone && <p className="text-danger">{errors.phone.message}</p>}
              </div>

              <div className="mb-3">
                <label className="form-label"><FaComment className="me-2" />Message</label>
                <textarea className='form-control btncolor'   {...register('message')} rows={4} placeholder="Enter Your Message"></textarea>
